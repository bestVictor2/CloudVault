...
// ResolvePreviewObject returns the bucket/object/name for secure preview streaming.
func ResolvePreviewObject(ctx context.Context, userID, fileID uint64) (string, string, string, error) {
	file, obj, err := loadPreviewFileAndObject(userID, fileID)
	if err != nil {
		return "", "", "", err
	}
	if shouldTranscodeForPreview(file.Name) && configPreviewTranscodeEnabled() {
		...
		return obj.BucketName, outputObject, outputName, nil
	}
	return obj.BucketName, obj.ObjectName, file.Name, nil
}
